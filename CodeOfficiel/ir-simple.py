...
def rank_docs(docs, query, index):
    ...
    def score(docID):
        """VOTRE CODE ICI
           ...
           Calculez un score de pertinence pour le document avec ID `docID`
           par rapport au question `query`, en se basant soit sur l'index
           `index` (plus efficace) ou soit directement sur les documents
           `docs`.

        """
        return 0
    
    # On prend les IDs de tout les documents.
    docIDs = list(docs.keys())

    # On trie selon la fonction score.
    docIDs.sort(key = score)
